...
class Generator(ABC):
    ...
    def write_ops(pybind_fn_names, ops_fn_defns, ops_macro, filename):
        s = "#pragma once\n"
        s = '#include <torch/all.h>\n\n'
        s += f"#define {ops_macro}\\\n"
        for fn_name in pybind_fn_names:
            s += f' ops.def("{fn_name}(Tensor! out, Tensor a, Tensor b, Tensor a_scales, Tensor b_scales) -> ()"); \\\n'
            s += f' ops.impl("{fn_name}", torch::kCUDA, &cutlass_scaled_mm); \\\n'
        s += "\n"
    ...
    @staticmethod
    def write_ops_pybind(pybind_fn_names, ops_fn_defns, ops_macro, filename):
        s = "#pragma once\n"
...
class Cutlass3xGenerator(Generator):
    ...
    SCRIPT_DIR=Path(os.path.dirname(os.path.realpath(__file__)))
    GENERATE_DIR= SCRIPT_DIR / "generated"
    FN_DEFN_JINJA= SCRIPT_DIR / "scaled_mm_c3x.jinja"
    FN_DECL_JINJA= SCRIPT_DIR / "scaled_mm_fnprototype.jinja"
    ...
    def generate_name(
        ...
        return 'autogen_cutlass3x_scaled_mm_sm{}_{}x{}x{}_{}x{}x{}_{}_{}_{}_{}_{}_{}'.format(
                arch,
                tile_shape[0], tile_shape[1], tile_shape[2],
    ...
    def generate_filename(
        ...
        f = '{}/autogen_cutlass_scaled_mm_c3x_{}x{}x{}_{}x{}x{}_{}_{}_{}_{}_{}_{}_{}'.format(
                Cutlass2xGenerator.GENERATE_DIR,
                tile_shape[0], tile_shape[1], tile_shape[2],
                cluster_shape[0], cluster_shape[1], cluster_shape[2],
                Generator.last_namespace(kernel_schedule),
...
def generate_cutlass2x_kernels():
    Cutlass2xGenerator.generate()

def generate_cutlass3x_kernels():
    # Cutlass3xGenerator.generate(Cutlass3xArgsTileClusterListI8)
